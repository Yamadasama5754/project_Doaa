...
void main() {
    ...
    if (u < 1.0) {
        ...
        // White hole: centre emits intense blue-white radiation
        float depth = clamp((u - 1.0) * 2.0, 0.0, 1.0);
        float core  = clamp((u - 1.0) * 8.0, 0.0, 1.0);
        vec4 glow   = vec4(0.7 + 0.3*core, 0.85 + 0.15*core, 1.0, 0.0);
        color += glow * (1.5 + 3.0*core);
    }
